...
def tela_inicial(tela, largura_tela, altura_tela, fundo_imagem):
    ...
    jogar_rect = jogar_texto.get_rect(center=(largura_tela / 2, altura_tela / 2))
    sair_rect = sair_texto.get_rect(center=(largura_tela / 2, altura_tela / 2 + 70))
    ...
    while True:
        ...
        tela.blit(titulo_texto, (largura_tela / 2 - titulo_texto.get_width() / 2, 100))
...
def tela_game_over(tela, largura_tela, altura_tela, fundo_imagem, jogador):
    ...
    mensagem_pontuacao = pygame.font.SysFont(None, 50).render(f"Pontuação: {jogador.pontos}", True, (255, 255, 255))

    # Define as opções de reiniciar ou sair do jogo
    mensagem_reiniciar = pygame.font.SysFont(None, 30).render("Pressione R para reiniciar o jogo", True, (255, 255, 255))
    mensagem_sair = pygame.font.SysFont(None, 30).render("Pressione Q para sair do jogo", True, (255, 255, 255))

    # Define as posições das mensagens na tela
    mensagem_game_over_rect = mensagem_game_over.get_rect()
    mensagem_game_over_rect.center = (largura_tela // 2, altura_tela // 2 - 100)
...
def jogo():
    ...
    while True:
        ...
        colisoes_inimigos = pygame.sprite.spritecollide(jogador, grupo_inimigos, True)
        ...
        colisoes_acoes = pygame.sprite.spritecollide(jogador, grupo_acoes, True)
        for acao in colisoes_acoes:
            jogador.pontos += 5

        # Atualiza os pontos do jogador na tela
        pontos_texto = pygame.font.SysFont(None, 30).render(f"Pontos: {jogador.pontos}", True, (255, 255, 255))
